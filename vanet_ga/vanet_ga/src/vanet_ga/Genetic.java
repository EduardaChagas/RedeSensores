...
public class Genetic {
	...
	public Genetic(int nGenerations, int popSize, int nRsus, int time, Double crossProb, Double mutProb, ArrayList<ArrayList<Integer>> m, Entropy e) {
	...
	private void initPopulation() {
		...
		int greedyPopulation = populationsSize / 4;
		int randPopulation = populationsSize/2;
		int entropyPopulation = populationsSize - randPopulation;
		numberOfIntersections = matrix.size();
		numberOfVehicles = matrix.get(0).size();
		
		
//		ArrayList<Integer> rsus = new ArrayList<Integer>();
//		rsus.add(12);
//		rsus.add(16);
//		rsus.add(26);
//		rsus.add(37);
...
//		ind.setRsus(rsus);
//		System.out.println("fitnes: "+ind.calcFitness(numberOfVehicles, numberOfRSUs, matrix, iTime));
		
		//System.out.println(greedyPopulation+"-"+randPopulation);

		for (int i = 0; i < randPopulation; i++) {
			population.add(generateRandIndividual(numberOfIntersections));
		}

		int pool = numberOfRSUs*2>numberOfIntersections?numberOfIntersections:numberOfRSUs*2;
		
		for(int i = 0; i<entropyPopulation;i++){
			Individual.individualByEntropy(entropy, pool, numberOfRSUs, randomGenerator);
		...
		greedy = new Greedy(matrix, numberOfRSUs, iTime, false, randomGenerator);
		for (int i = 0; i < greedyPopulation; i++) {
			population.add(greedy.generateGreedyIndividual());
		}
	...
	public Double evolve() {
		...
		while (numberOfGenerations > 0) {
			...
			for(Individual ind : childs){
				ind.mutate(mutationProb, randomGenerator, numberOfIntersections,entropy);
...
//			}
		...
		fittest.print();
		return fittest.calcFitness(numberOfVehicles, numberOfRSUs, matrix, iTime);
	}
