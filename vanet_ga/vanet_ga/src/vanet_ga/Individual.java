...
public class Individual{
	...
	public void mutate(double probability, Random rg, int nIntersections, Entropy e){
		

		mutateByEntropy(probability,rg,e);
...
//		}
	...
	private void mutateByEntropy(double probability, Random rg, Entropy e){
		int nRsus = rsus.size();
		...
		if(mutate<probability){
			...
			int nextGen = 0;
			int newGen = e.getMax(nextGen);
			while(contains(newGen)){
				nextGen++;
				newGen = e.getMax(nextGen);
			}
	...
	public static Individual individualByEntropy(Entropy e, int poolSize, int nRsus, Random randomGenerator){
		ArrayList<Integer> rsus = new ArrayList<Integer>();
		Individual ind = new Individual();
		while(ind.rsus.size()<nRsus){
			int rsu = randomGenerator.nextInt(poolSize-1);
			while(rsus.indexOf(rsu)!=-1){
				rsu = randomGenerator.nextInt(poolSize-1);
			}
			ind.add(e.getMax(rsu));
		}
		return ind;
	}
	...
	public void setRsus(ArrayList<Integer> r){
		this.rsus = r;
	}
}
